    ...
    return (
      <div className="rounded-2xl border border-primary/30 bg-primary/5 p-5">
        ...
        <div className="flex items-center gap-2 mb-3">
          <input value={ctText(data.question)} onChange={(e) => onChange({ ...data, question: mkCT(e.target.value, ctColor(data.question)) })} placeholder="¿Cuál es la pregunta?" className="flex-1 text-lg font-bold bg-background border border-border rounded-xl px-4 py-3 outline-none focus:border-primary" style={{ color: ctColor(data.question) }} />
  ...
  Renderer: ({ data, blockId }) => {
    ...
    return (
      <div className="my-6 rounded-2xl border border-primary/30 bg-primary/5 p-6">
        <p className="text-lg font-semibold text-foreground mb-1" style={{ color: ctColor(data.question) }}>{ctText(data.question)}</p>
