...
export const QuizBlock = defineBlock<QuizData>({
  ...
  Editor: ({ data, onChange }) => {
    const setOpt = (i: number, v: CText) => onChange({ ...data, options: data.options.map((o, idx) => (idx === i ? v : o)) });
    ...
    return (
      <div className="rounded-2xl border border-secondary/30 bg-secondary/5 p-5">
        ...
        <div className="space-y-2">
          {data.options.map((opt, i) => (
            <div key={i} className="flex items-center gap-2 bg-background/50 rounded-lg border border-border/50 p-2">
              ...
              <input value={ctText(opt)} onChange={(e) => setOpt(i, mkCT(e.target.value, ctColor(opt)))} placeholder={`Opción ${i + 1}`} className="flex-1 bg-transparent outline-none text-sm" style={{ color: ctColor(opt) }} />
              <ColorDots color={ctColor(opt)} onPick={(c) => setOpt(i, mkCT(ctText(opt), c))} />
        ...
        <div className="flex items-center gap-2 mt-3">
          <input value={ctText(data.feedback)} onChange={(e) => onChange({ ...data, feedback: mkCT(e.target.value, ctColor(data.feedback)) })} placeholder="Retroalimentación al responder (opcional)" className="flex-1 text-sm bg-background border border-border rounded-lg px-3 py-2 outline-none focus:border-secondary" style={{ color: ctColor(data.feedback) }} />
          <ColorDots color={ctColor(data.feedback)} onPick={(c) => onChange({ ...data, feedback: mkCT(ctText(data.feedback), c) })} />
        </div>
      </div>
    );
  },
  Renderer: ({ data, blockId }) => {
    ...
    return (
      <div className="my-6 rounded-2xl border border-secondary/30 bg-secondary/5 p-6">
          ...
          {data.options.map((opt, i) => {
            ...
            return (
              <button key={i} disabled={checked} onClick={() => setSelected(i)} className={`w-full flex items-center gap-3 p-3 rounded-lg border text-left transition-colors ${isCorrect ? 'border-green-500 bg-green-500/10' : isWrong ? 'border-red-500 bg-red-500/10' : selected === i ? 'border-secondary bg-secondary/10' : 'border-border hover:border-secondary/50 bg-background/40'}`}>
                ...
                <span className="flex-1 text-sm" style={{ color: ctColor(opt) }}>{ctText(opt)}</span>
        ...
        {checked && ctText(data.feedback) && <p className="mt-3 text-sm text-foreground/70 italic" style={{ color: ctColor(data.feedback) }}>{ctText(data.feedback)}</p>}
      </div>
    );
  },
});
