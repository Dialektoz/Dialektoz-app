...
export const OrderingBlock = defineBlock<OrderingData>({
  ...
  Editor: ({ data, onChange }) => {
    const setItem = (i: number, v: CText) => onChange({ ...data, items: data.items.map((it, idx) => (idx === i ? v : it)) });
    ...
    return (
      <div className="rounded-2xl border border-primary/30 bg-primary/5 p-5">
        ...
        <div className="flex items-center gap-2 mb-3">
          <input value={ctText(data.prompt)} onChange={(e) => onChange({ ...data, prompt: mkCT(e.target.value, ctColor(data.prompt)) })} placeholder="Instrucción (ej: Ordena los pasos)" className="flex-1 font-semibold bg-background border border-border rounded-xl px-4 py-3 outline-none focus:border-primary" style={{ color: ctColor(data.prompt) }} />
        ...
        <div className="space-y-2">
          {data.items.map((it, i) => (
            <div key={i} className="flex items-center gap-2 bg-background/50 rounded-lg border border-border/50 p-2">
              ...
              <input value={ctText(it)} onChange={(e) => setItem(i, mkCT(e.target.value, ctColor(it)))} placeholder={`Elemento ${i + 1}`} className="flex-1 bg-transparent outline-none text-sm" style={{ color: ctColor(it) }} />
              <ColorDots color={ctColor(it)} onPick={(c) => setItem(i, mkCT(ctText(it), c))} />
  ...
  Renderer: ({ data, blockId }) => {
    ...
    const [order, setOrder] = useState<CText[]>(() => {
      const s = shuffle(correct);
      // avoid starting already-solved
      return s.every((v, i) => ctText(v) === ctText(correct[i])) && correct.length > 1 ? shuffle(correct) : s;
    });
    ...
    return (
      <div className="my-6 rounded-2xl border border-primary/30 bg-primary/5 p-6">
        ...
        <div className="space-y-2">
          {order.map((item, i) => {
            const ok = checked && ctText(item) === ctText(correct[i]);
            const bad = checked && ctText(item) !== ctText(correct[i]);
            return (
              <div
                key={ctText(item) + i}
                draggable={!checked}
              ...
              >
                ...
                <span className="flex-1 text-sm text-foreground" style={{ color: ctColor(item) }}>{ctText(item)}</span>
        ...
        <div className="mt-4 flex items-center gap-3">
          <button disabled={checked} onClick={() => { setChecked(true); report(order.every((v, i) => ctText(v) === ctText(correct[i]))); }} className="bg-primary text-primary-foreground font-bold text-sm px-4 py-2 rounded-lg disabled:opacity-50">Comprobar</button>
